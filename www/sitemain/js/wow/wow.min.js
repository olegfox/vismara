(function(){var MutationObserver,Util,WeakMap,getComputedStyle,getComputedStyleRX,__bind=function(fn,me){return function(){return fn.apply(me,arguments);};},__indexOf=[].indexOf||function(item){for(var i=0,l=this.length;i<l;i++){if(i in this&&this[i]===item)return i;}return-1;};Util=(function(){function Util(){}
    ...
    WOW.prototype.init=function(){var _ref;this.element=window.document.documentElement;if((_ref=document.readyState)==="interactive"||_ref==="complete"){this.start();}else{this.util().addEvent(document,'DOMContentLoaded',this.start);}
        ...
        if(!this.disabled()){this.util().addEvent(window,'scroll',this.scrollHandler);$('.page-wrapper').scroll(this.scrollHandler);$('.page-wrapper-services').scroll(this.scrollHandler);$('.page-wrapper-about').scroll(this.scrollHandler);this.util().addEvent(window,'resize',this.scrollHandler);this.interval=setInterval(this.scrollCallback,50);}
        ...
        return top;};WOW.prototype.isVisible=function(box){var bottom,bottom2,offset,top,top2,viewBottom,viewTop,viewBottom2,viewTop2;offset=box.getAttribute('data-wow-offset')||this.config.offset;viewTop=window.pageYOffset;viewBottom=viewTop+Math.min(this.element.clientHeight,this.util().innerHeight())-offset;viewTop2=0;viewBottom2=viewTop2+Math.min(this.element.clientHeight,this.util().innerHeight())-offset;top=this.offsetTop(box);top2=$(box).position().top;bottom=top+box.clientHeight;bottom2=$(box).position().top+box.clientHeight;return(top<=viewBottom&&bottom>=viewTop)||(top2<=viewBottom2&&bottom2>=viewTop2);};WOW.prototype.util=function(){return this._util!=null?this._util:this._util=new Util();};WOW.prototype.disabled=function(){return!this.config.mobile&&this.util().isMobile(navigator.userAgent);};return WOW;})();}).call(this);